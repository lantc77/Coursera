...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty()) {
			...
			List<String> oneAwayList = nw.distanceOne(currWord, true);
			for(String oneAwayWord: oneAwayList) {
				if(!visited.contains(oneAwayWord)) {
					visited.add(oneAwayWord);
					WPTreeNode node = new WPTreeNode(oneAwayWord, currNode);
					if(oneAwayWord.equals(word2)) 
	...
	public List<String> findShortestPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty()) {
			...
			for(String oneAwayWord: oneAwayList) {
				if(!visited.contains(oneAwayWord)) {
					...
					WPTreeNode node = new WPTreeNode(oneAwayWord, currNode);
					if(oneAwayWord.equals(word2)) {
						List<String> list = node.buildPathToRoot();
						if(retList.isEmpty() || list.size() < retList.size()) {
							retList = list;
							pathNum ++;
						}
						break;
		...
		System.out.println("the number of path: "+pathNum);
		return retList;
	}
	...
	public static void main(String args[]) {
		...
		String word1 = "angle";
		String word2 = "apple";
		List<String> path = test.findShortestPath(word1, word2);
